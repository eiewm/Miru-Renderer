    ...
    while press_idx < presses.len() {
        ...
        let prev_miss_shor_stays = true
                ...
                .any(|next_pt| {
                    ...
                    next_press > ho.end_time.unwrap_or(ho.time)
                        ...
                        && events
                            ...
                            .map(|ev| {
                                let next_pair_prewin = col_notes
                                    .get(note_pos + 1)
                                    .map(|(_, next_ho)| {
                                        next_ho.is_long_note()
                                            && next_press >= next_ho.time - w.hit50
                                            && next_press < next_ho.time
                                            && ev.time > next_press
            ...
            && col_notes
                ...
                .unwrap_or(false);
        let prev_miss_short_stays = true
            && ho.is_long_note()
            && prev_is_ln_stale
            && prev_was_miss
            && prev_break_pre
        ...
        let prev_miss_long_stays = true
            ...
            && col_notes
                ...
                .unwrap_or(false);
        let prev_closed_tail = true
            && ho.is_long_note()
            && prev_is_ln_stale
            && prev_was_miss
            && !prev_had_prewin_pen
            && same_as_prev_note_pt
            && cand_pt < ho.time
            && !holds_through_head
            && prev_end_stale
                .map(|prev_end| cand_pt >= prev_end - (w.max + 4) && cand_pt <= prev_end)
                .unwrap_or(false)
            && release_after_cand
                .zip(prev_end_stale)
                .map(|(rt, prev_end)| rt > prev_end && rt < ho.time && rt <= prev_end + w.hit300)
                .unwrap_or(false)
            && presses
                .iter()
                .skip(press_idx + 1)
        ...
        } else if prev_closed_tail {
            Some("prev_closed_tail")
        } else {
            None
        };
