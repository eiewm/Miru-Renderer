...
pub(super) fn scan(
    ...
    if let Some(pt) = tail_eval_press_time {
        ...
        let hless_starts_closed = true
            && !head_was_hit
            && press_time.is_none()
            && tail_only_pt == Some(pt)
            && exact_rel_at_press;
        let prhd_exact_tail_meta =
            true && head_was_hit && pt < ho.time && ln_duration < w.hit50 * 2 && exact_rel_at_press;
        if (false || prhd_exact_tail_meta) && exact_rel_at_press {
        ...
        let pre_ln_nea_head_repr = false
            && prev_same_col_is_ln
            && pt >= ho.time
            && pt - ho.time > w.max
            && pt - ho.time <= w.hit300;
                ...
                let valid_pre_head_pair = pre_head_press
                    .map(|pre_press_time| {
                        ...
                        let pre_head_pt_off = prev_same_col_time
                            ...
                            .unwrap_or(true);
                        let prev_ln_pre_rel_end = if prev_same_col_is_ln && false {
                            prev_same_end
                                .map(|prev_end| {
        ...
        let prehead_counts_break = true
            && !head_was_hit
            && press_time
                .zip(tail_only_pt)
                .map(|(head_pt, tail_only_pt)| {
                    pt == tail_only_pt
                        && head_pt < ho.time
                        && head_pt < tail_only_pt
                        && events.iter().any(|ev| !ev.pressed && ev.time == head_pt)
                })
                .unwrap_or(false);
        if prehead_counts_break {
            if let Some(head_pt) = press_time {
                imm_rel_at_press = Some(head_pt);
                has_early_rel = true;
                if first_early_rel.map(|t| head_pt < t).unwrap_or(true) {
                    first_early_rel = Some(head_pt);
                }
            }
            repr_after_rel = true;
            if first_repr_after_rel.map(|rp| pt < rp).unwrap_or(true) {
                first_repr_after_rel = Some(pt);
            }
            last_repr_time = Some(pt);
        }
        ...
        if hless_starts_closed {
            segments.push((seg_start, Some(seg_start)));
            down = false;
        }
        for ev in events
            .iter()
        ...
        if let Some(rp) = first_repr_after_rel {
            rel_after_repr = events
                .iter()
                .find(|e| e.time > rp && !e.pressed)
                .map(|e| e.time);
        }
        ...
        let near_head_repr_win = if false { w.hit200 } else { w.hit300 };
            ...
            && !keep_hit_repr
        {
            ...
            let long_post_pref_tail = true
                ...
                && first_repr_after_rel
                    ...
                    .unwrap_or(false);
            if pref_repr_post_rel.is_none()
                && firs_repr_needs_resc
                && (false || long_post_pref_tail)
            {
                if let (Some(first_rp), Some(last_rp)) = (first_repr_after_rel, last_repr_time) {
                    if last_rp > first_rp && last_rp <= end_time {
                        let rel_post_last_repr = events
                            .iter()
                            .find(|e| e.time > last_rp && !e.pressed)
                            .map(|e| e.time);
                        let last_pre_tail_stays = next_same_col_time
                            .map(|next_t| {
                                last_rp < next_t
                                    && rel_post_last_repr.map(|rt| rt < next_t).unwrap_or(false)
                            })
                            .unwrap_or(true);
                        let last_pre_tail_claim = judgments.iter().any(|jj| {
                            jj.index != idx
                                && jj.column == ho.column
                                && jj.press_time == Some(last_rp)
                        });
                        if rel_post_last_repr
                            .map(|rt| rt < tail_start)
                            .unwrap_or(false)
                            && (false || (!last_pre_tail_claim && last_pre_tail_stays))
                        {
                            pref_repr_post_rel = Some(last_rp);
                        }
                    }
                }
            }
        ...
        let long_hit_maybe_bridge = true
            && head_was_hit
            && strong_head_hit
            && has_early_rel
            ...
            && first_repr_after_rel
                ...
                .unwrap_or(false);
        let tail_pref_long_scaled = true
            && head_was_hit
            && strong_head_hit
            && has_early_rel
            && ln_duration > late_body_lim
        ...
        let tail_pref_long_expnd = true
            ...
            && first_repr_after_rel
                ...
                .unwrap_or(false);
        let short_hit_guard = true
            && head_was_hit
            && strong_head_hit
            && has_early_rel
            && ln_duration <= late_body_lim
            && first_early_rel
                .map(|t| t > ho.time && t < tail_start)
                .unwrap_or(false)
            && first_repr_after_rel
                .zip(rel_after_repr)
                .map(|(rp, rr)| {
                        ...
                        first_repr_after_rel
                            ...
                            .map(|(rp, rr)| {
                                *seg_start > rp
                                    ...
                                    && seg_end_time > rr
                                    && seg_end_time < late_repr_guard
                            })
                            .unwrap_or(false)
                    })
                    .unwrap_or(false)
            });
        let tail_pref_early_maybe = true
            && head_was_hit
            && head_is_h100
        ...
        let tai_pre_lon_pre_frag = true
            ...
            && ln_duration > late_body_lim
            && first_early_rel
                .map(|t| t < ho.time && t <= tail_start - w.hit50)
                .unwrap_or(false)
            && first_repr_after_rel
                .zip(rel_after_repr)
                .map(|(rp, rr)| {
        ...
        let tail_pref_exact_maybe = true
            && head_was_hit
            && head_is_h50
            && press_time.map(|pt| pt < ho.time).unwrap_or(false)
            && has_early_rel
            ...
            || tail_pref_exact_maybe
        {
            ...
            for (seg_start, seg_end) in &segments {
                ...
                if short_hit_guard
                    && (*seg_start >= late_repr_guard || seg_end_time >= late_repr_guard)
                {
                    continue;
                }
                    ...
                    || tail_pref_exact_maybe
                {
                    let seg_stays_pre_next = next_same_col_time
                        .map(|next_t| *seg_start < next_t - w.hit50 && seg_end_time < next_t)
                        .unwrap_or(true);
                    let h50_end_seg_prom = (tai_pre_lon_pre_frag
                        || tail_pref_frag
                        || tail_pref_exact_maybe)
                        && seg_end_time == end_time
                ...
                pref_late_body_exact =
                    (tai_pre_lon_pre_frag || tail_pref_frag || tail_pref_exact_maybe)
                        && seg_end_time == end_time
                        && (next_same_col_time
                            .map(|next_t| *seg_start < next_t - w.hit50 && seg_end_time < next_t)
                            .unwrap_or(true)
            ...
            if let Some((preferred_rp, preferred_rr)) = pref_late_body_pair {
                ...
                } else if tai_pre_lon_pre_frag || tail_pref_frag || tail_pref_exact_maybe {
        ...
        let prehead_tail_rec = true
            ...
            && first_repr_after_rel
                .zip(rel_after_repr)
                .map(|(rp, rr)| {
                    rp >= ho.time && rp <= ho.time + w.hit300 && rr >= tail_start && rr < end_time
                })
                .unwrap_or(false)
            && first_repr_after_rel
                .zip(rel_after_repr)
                .map(|(first_rp, first_rr)| {
                    segments.iter().any(|(seg_start, seg_end)| {
                        seg_end
                            .map(|seg_end_time| {
                                *seg_start > first_rp
                                    && *seg_start <= end_time
                                    && seg_end_time > first_rr
                                    && seg_end_time < end_time
                            })
                            .unwrap_or(false)
                    })
                ...
                .unwrap_or(false);
        if prehead_tail_rec {
            let cur_tail_post_repr = rel_after_repr.unwrap_or(i32::MIN);
            let mut pref_late_body_pair: Option<(i32, i32)> = None;
            for (seg_start, seg_end) in &segments {
                let Some(seg_end_time) = *seg_end else {
                    continue;
                ...
                if *seg_start > end_time
                    || seg_end_time <= cur_tail_post_repr
                    || seg_end_time >= end_time
                {
                    continue;
                }
                let seg_stays_pre_next = next_same_col_time
                    .map(|next_t| *seg_start < next_t && seg_end_time < next_t)
                    .unwrap_or(true);
                let claimed_by_other_note = judgments.iter().any(|jj| {
                    jj.index != idx && jj.column == ho.column && jj.press_time == Some(*seg_start)
                });
                ...
                if claimed_by_other_note || steals_imm_ln || !seg_stays_pre_next {
                    continue;
                }
                pref_late_body_pair = Some((*seg_start, seg_end_time));
            }
            if let Some((preferred_rp, preferred_rr)) = pref_late_body_pair {
                first_repr_after_rel = Some(preferred_rp);
                last_repr_time = Some(preferred_rp);
                rel_after_repr = Some(preferred_rr);
            }
        }
    } else {
