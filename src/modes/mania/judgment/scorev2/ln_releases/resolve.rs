...
pub(super) fn resolve(
    ...
    let mut _prehead_h100_claims = false;
    ...
    let near_tail_pref_repr = true
        ...
        && first_repr_after_rel
            ...
            .unwrap_or(false);
    let near_tail_claims_repr = true
        && head_was_hit
        && has_early_rel
        && first_early_rel
    ...
    let hea_mis_rep_tail_rec = true
        ...
        && first_repr_after_rel
            ...
            .unwrap_or(false);
    let short_miss_exact_repr = true
        && !head_was_hit
        && press_time.is_some()
        && has_early_rel
        && first_early_rel.map(|t| t < ho.time).unwrap_or(false)
        ...
        || miss_repr_tail_any
    {
        for (seg_start, seg_end) in segments {
            ...
            let tail_open_stays_miss = true
                ...
                && segment_open
                && open_seg_late_over;
            let head_hit_late_miss = true
                && head_was_hit
                && !has_early_rel
                && raw_rel_over >= tail_end_exclusive
                && tail_only_pt.is_none();
            let late_open_prewin = true
                && !has_early_rel
                && segment_open
                && open_seg_late_over
                && first_rel_after_press
                ...
                || raw_release >= tail_end_exclusive
            {
                ...
                if late_rel_auto_h50 {
                    ...
                    (clamped, clamped)
                } else if true && tail_only_pt.is_some() && !has_early_rel && open_seg_late_over {
            ...
            let first_repress_segment = first_repr_after_rel
                .map(|rp| *seg_start == rp)
                .unwrap_or(false);
            let first_repr_to_next_ln = true
                && first_early_rel.map(|t| *seg_start > t).unwrap_or(false)
            ...
            let fir_repr_to_next_tap = if true
                && !head_was_hit
                && press_time.is_none()
                && tail_only_pt.is_some()
                && first_early_rel.map(|t| *seg_start > t).unwrap_or(false)
            ...
            let er_mis = first_early_rel
                .map(|rt| {
                    calc_rel_kind((rt - end_time).abs(), w, tail_window_scale) == ReleaseKind::Miss
                })
                .unwrap_or(false);
            ...
            let firs_repr_to_shor_ln = if true
                && !head_was_hit
                && press_time.map(|pt| pt < ho.time).unwrap_or(false)
                && first_early_rel.map(|t| *seg_start > t).unwrap_or(false)
                && first_repress_segment
                    ...
                    .and_then(|(next_idx, next_ho)| {
                        map.hit_objects[(next_idx + 1)..]
                            ...
                            .map(|(offset, next_next_ho)| {
                                (next_idx, next_ho, next_idx + 1 + offset, next_next_ho)
                            })
                    })
                    .map(|(next_idx, next_ho, next_next_idx, next_next_ho)| {
                        ...
                        let next_tail_end =
                            next_end_time + ((w.hit100 as f32) * tail_window_scale).round() as i32;
                        let can_to_imm_nex_sho_ln = judgments.iter().any(|jj| {
                            jj.index == next_idx
                                && jj.column == ho.column
                            ...
                            && effective_release > next_ho.time
                            && effective_release < next_end_time
                            && effective_release <= next_ho.time + w.hit300
                            && effective_release >= next_tail_start
                            && effective_release < next_tail_end;
                        let Some(replacement_press) =
                        ...
                        let next_next_tail_start = next_next_end_time
                            - ((w.hit50 as f32) * tail_window_scale).round() as i32;
                        let next2_tail_end = next_next_end_time
                            + ((w.hit100 as f32) * tail_window_scale).round() as i32;
                        let replacement_rel_time = events
                            .iter()
                            .find(|ev| ev.time > replacement_press && !ev.pressed)
                            .map(|ev| ev.time);
                        let repl_claim_by_fol_ln = judgments.iter().any(|jj| {
                            jj.index == next_next_idx
                                && jj.column == ho.column
                                && jj.press_time == Some(replacement_press)
                                && matches!(jj.kind, JudgmentKind::Max | JudgmentKind::Hit300)
                        });
                        can_to_imm_nex_sho_ln
                            && replacement_press > next_end_time
                            && !matches!(replacement_kind, JudgmentKind::Max | JudgmentKind::Hit300)
                            && repl_claim_by_fol_ln
                            && replacement_rel_time
            ...
            if firs_repr_to_shor_ln {
                rel_time = first_early_rel;
                end_diff = first_early_rel.map(|rt| (rt - end_time).abs()).unwrap_or(0);
                let yildd_first_rel_kind = calc_rel_kind(end_diff, w, tail_window_scale);
                rel_kind = if matches!(yildd_first_rel_kind, ReleaseKind::Miss | ReleaseKind::None)
                {
            ...
            if (miss_repr_tail_any || tail_pref_post_repr) && first_repress_segment {
                let reclaim_conflict =
                    reclaim_pt_conflict(judgments, map, events, idx, ho.column, *seg_start, w);
                if reclaim_conflict.competing_note_idx.is_some() {
                    state.reclaim.claimed_pt = reclaim_conflict.claimed_press_time;
                    state.reclaim.competing_idx = reclaim_conflict.competing_note_idx;
                    state.reclaim.competing_time = reclaim_conflict.competing_note_time;
                    state.reclaim.followup_pt = reclaim_conflict.followup_press_time;
                    state.reclaim.next_win_start = reclaim_conflict.next_window_start;
                    state.reclaim.competing_kind = reclaim_conflict.competing_note_kind;
                }
                if reclaim_conflict.press_first_rel {
                    rel_time = first_early_rel;
                    end_diff = first_early_rel.map(|rt| (rt - end_time).abs()).unwrap_or(0);
                    rel_kind = ReleaseKind::Hit50;
                    force_kind = false;
                    repr_hit_tail = false;
                    _prehead_h100_claims = true;
                    break;
                }
            }
    ...
    let pre_tail_quick_miss = pre_tail_frag_h50
        && rel_time.map(|rt| rt < end_time).unwrap_or(false)
        && tail_only_pt
            .map(|pt| end_time - pt <= w.hit50 + w.max)
            .unwrap_or(false)
        && tail_only_pt
            .and_then(|pt| {
                events
                    .iter()
                    .take_while(|ev| ev.time < pt)
                    .filter(|ev| !ev.pressed && ev.time >= ho.time + w.hit50)
                    .map(|ev| pt - ev.time)
                    .last()
            })
            .map(|gap| gap <= w.max + 4)
            .unwrap_or(false);
    if pre_tail_quick_miss {
        rel_time = None;
        rel_kind = ReleaseKind::Miss;
        end_diff = 0;
        force_kind = false;
    } else if pre_tail_frag_h50 {
        rel_kind = ReleaseKind::Hit50;
        force_kind = false;
    }
    let term_long_stays_miss = true
        && head_was_hit
        && !has_early_rel
