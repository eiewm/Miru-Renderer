...
pub(super) fn resolve_primary(ctx: &PressNoteCtx<'_>, state: &mut PressState) {
    ...
    let mut ghost_prev_early = false;
    ...
    if let Some(pt) = early_pen_pt {
        ...
        let prev_note_is_ln = prev_note
            ...
            .unwrap_or(false);
        let prev_note_time = prev_note.map(|(_, prev_ho)| prev_ho.time);
        let prev_note_end_time =
            prev_note.map(|(_, prev_ho)| prev_ho.end_time.unwrap_or(prev_ho.time));
        let prev_note_duration = prev_note_end_time
            .zip(prev_note.map(|(_, prev_ho)| prev_ho.time))
            .map(|(prev_end, prev_time)| prev_end - prev_time);
        let early_press_rel_time = events
            .iter()
            .find(|ev| ev.time > pt && !ev.pressed)
            .map(|ev| ev.time);
        let short_overlap_miss = if false && ho.is_long_note() {
        ...
        if !short_overlap_miss {
            ...
            if !cont_prewin_chain {
                ...
                let has_in_win_cand = press_idx < presses.len()
                    ...
                    && presses[press_idx] < lock_end_exclusive
                    && !reserved_ln_repr.contains(&presses[press_idx]);
                let tap_frag_stays_ghost = !ho.is_long_note()
                    && prev_note_is_ln
                    && !prev_was_miss
                    && !prev_had_prewin_pen
                    && prev_note_duration
                    ...
                    && prewindow_overflow > legacy_early_win
                    && early_press_rel_time
                        .map(|rt| rt > pt && rt < ho.time)
                        .unwrap_or(false)
                    && prev_col_pt
                        .zip(prev_note_end_time)
                        .map(|(prev_pt, prev_end)| {
                            events
                                .iter()
                                .find(|ev| ev.time > prev_pt && !ev.pressed)
                                .map(|ev| ev.time < prev_end && pt > ev.time && pt < prev_end)
                                .unwrap_or(false)
                        })
                        .unwrap_or(false);
                let cur_prev_early_ghost = !ho.is_long_note()
                    && !prev_note_is_ln
                    && !prev_was_miss
                    && !prev_had_prewin_pen
                    && !has_in_win_cand
                    ...
                    && calc_hit_kind((ho.time - pt).abs(), w) == JudgmentKind::Miss
                    && early_press_rel_time
                        .zip(next_note_time.zip(col_notes.get(note_pos + 1)))
                        .map(|(rt, (next_t, (_, next_ho)))| {
                            !next_ho.is_long_note()
                                ...
                                && {
                                    ...
                                    next_pt >= next_t - w.hit50
                                        && next_pt < next_t + w.hit100
                        ...
                        .unwrap_or(false)
                    || early_press_rel_time
                        .zip(prev_col_pt.zip(prev_note_time))
                        .map(|(rt, (prev_pt, prev_t))| {
                            prev_pt < prev_t
                                && pt > prev_pt
                                && pt < prev_t
                                && rt > prev_t
                        ...
                        .unwrap_or(false);
                if tap_frag_stays_ghost {
                    press_time = None;
                } else if cur_prev_early_ghost {
                    press_time = None;
                    early_pen_pt = None;
                    ghost_prev_early = true;
                }
                ...
                let pre_frag_late_follow = !false
                    && !ho.is_long_note()
                    && press_idx < presses.len()
                    && presses[press_idx] >= window_start
                    && presses[press_idx] < lock_end_exclusive
        ...
        let short_post_end_tap = if false && ho.is_long_note() && ln_duration <= w.hit100 {
            if let Some((_, next_ho)) = col_notes.get(note_pos + 1) {
        ...
        let short_post_pref_short = if ho.is_long_note()
            ...
            && pt >= ho.time + w.hit50
        {
            if let Some((_, next_ho)) = col_notes.get(note_pos + 1) {
                if !next_ho.is_long_note() {
                    ...
                    let next_window_start = next_ho.time - w.hit50;
                    let next_head_kind = calc_hit_kind((pt - next_ho.time).abs(), w);
                    let next_next_note_time = col_notes.get(note_pos + 2).map(|(_, n)| n.time);
                    let next_ln_late_end = next_next_note_time
                        .map(|next_time| next_time <= next_ho.time + w.hit50)
                        .unwrap_or(false);
                    ...
                    next_duration <= w.hit100
                        ...
                        && pt < next_ho.time
                        && next_head_kind == JudgmentKind::Hit200
                        && !has_next_pt_fol
                        && release_after_pt
        ...
        let en = ho.end_time.unwrap_or(ho.time);
        let du = en - ho.time;
        let er = events
            .iter()
            .filter(|ev| !ev.pressed && ev.time > ho.time && ev.time < pt)
            .map(|ev| ev.time)
            .max();
        let edg_nln = ho.is_long_note()
            && pt == ho.time + w.hit50
            && (du > w.hit100 || er == Some(en))
            && note_pos
                .checked_sub(1)
                .and_then(|p| col_notes.get(p))
            ...
            && col_notes
                .get(note_pos + 1)
                .map(|(_, n)| {
                    let nd = n.end_time.unwrap_or(n.time) - n.time;
                    n.is_long_note()
                        && nd > w.hit100
                        && pt >= n.time - w.hit50
                        && (pt >= n.time || (n.time - pt > w.hit300 && nd > w.hit100 + w.max / 2))
                })
                .unwrap_or(false);
        steals_next_ex = (exact_next_head_late && !short_exact_no_follow)
            || short_h50_pref_tap
            || short_post_end_tap
            || short_post_pref_short
            || edg_nln;
            ...
            if let Some((_, next_ho)) = col_notes.get(note_pos + 1) {
                if !next_ho.is_long_note() {
                    ...
                    let bound_press_next = rel_after_bound
                        ...
                        .unwrap_or(false);
                    let short_bound_stays = !false
                        && ln_duration <= w.hit50 * 2
                        && pt >= next_window_start
                        && pt < next_ho.time
                        && !has_next_pt_fol
                        && pt <= ho.end_time.unwrap_or(ho.time) + w.hit300
                        && rel_after_bound
                            .map(|rt| rt <= next_ho.end_time.unwrap_or(next_ho.time))
                            .unwrap_or(false);
                    let bound_press_to_short = !false
                        ...
                        && next_next_note_time
                            ...
                            .unwrap_or(false);
                    let has_follow_claim_cur = if false {
                        has_next_pt_fol
                    } else {
                        has_next_pt_fol && !bound_press_next && !bound_press_to_short
                    };
                    let bound_hold_cross_next = !false
                        && !has_next_pt_fol
                        && pt >= next_window_start
                        && pt < next_ho.time
                        && rel_after_bound.map(|rt| rt > next_ho.time).unwrap_or(true);
                    has_follow_claim_cur || bound_hold_cross_next || short_bound_stays
                } else if ln_duration <= w.hit100 {
                    let next_window_start = next_ho.time - w.hit50;
                    ...
                    let las_inn_rel_pre_boun = events
                        .iter()
                        .filter(|ev| !ev.pressed && ev.time > ho.time && ev.time < pt)
                        .map(|ev| ev.time)
                        .max();
                    let near_tail_pre_bound = las_inn_rel_pre_boun
                        .map(|rt| ho.end_time.unwrap_or(ho.time) - rt <= w.max)
                        .unwrap_or(false);
                    let exact_post_end_pref = !false
                        && pt > ho.end_time.unwrap_or(ho.time)
                        && pt < next_window_start
                        && first_rel_pt
                    ...
                    let next_long_bridge = !has_next_pt_fol
                        ...
                        && pt >= next_window_start
                        && pt < next_ho.time
                        && ((next_ho.time - pt <= w.max / 2) || near_tail_pre_bound)
                        && bound_press_next
                        && bound_rel_before_next;
                    let short_ln_bound_pref = las_inn_rel_pre_boun.is_some()
                        && !near_tail_pre_bound
                        && (!bound_rel_before_next || pt > ho.end_time.unwrap_or(ho.time));
    ...
    state.head_candidate.ghost_prev_early = ghost_prev_early;
}
