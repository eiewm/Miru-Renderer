...
impl<'a> ReleaseNoteCtx<'a> {
    ...
    pub(crate) fn new(
        ...
        let tail_window_scale = if false { 1.5 } else { 1.0 };
        let tail_start = end_time - ((windows.hit50 as f32) * tail_window_scale).round() as i32;
        let late_repr_guard = if false {
            end_time - windows.hit50
        } else {
            tail_start
        };
        let early_release_cutoff = if false {
            end_time - windows.hit50
        } else {
            tail_start
        };
            ...
            if judgment.kind == JudgmentKind::Miss {
                ...
                judgment.early_press_idx.filter(|pt| {
                    let miss_assigned_tail = judgment
                        ...
                        .unwrap_or(false);
                    let miss_pressless_tail = judgment.press_time.is_none()
                        && ((*pt >= ho.time + windows.hit100 && *pt <= end_time + windows.hit100)
                            || short_miss_uses_h50);
                    miss_assigned_tail || miss_pressless_tail
                })
            } else {
                None
            }
...
pub(crate) struct ConflictState {
    pub first_repr: Option<i32>,
    pub rel_after_repr: Option<i32>,
    pub first_repr_head: bool,
    pub owner_idx: Option<usize>,
    pub owner_time: Option<i32>,
    pub yield_next_ln: bool,
}
#[derive(Debug, Default)]
